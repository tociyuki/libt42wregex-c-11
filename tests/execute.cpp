...
void test11 (test::simple& ts)
{
    ...
    std::wstring str (L"number 3.1425 is the pi");
    std::vector<int> cap;
    int rc = re.exec (str, cap, 0);
    ts.ok (rc == 13, L"qr/.*([0-9]+(?:[.][0-9]*)?)/ =~ \"number 3.1425\"_\" is the pi\"");
    std::wstring m0(str.begin () + cap[0], str.begin () + cap[1]);
    std::wstring m1(str.begin () + cap[2], str.begin () + cap[3]);
    ts.ok (m0 == L"number 3.1425", L"$0 == \"number 3.1425\"");
    ts.ok (m1 == L"3.1425", L"$1 == \"3.1425\"");
}
